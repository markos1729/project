...
void parse_bindings(char *s,unsigned int &npredicates,predicate* &predicates,unsigned int &nfilters,filter* &filters) {
	...
	while (token!=NULL) {
		if (strcnt(token,'.')==2) {
			sscanf(token,"%u.%u=%u.%u",&predicates[npredicates].rela_id,&predicates[npredicates].cola_id,
									   &predicates[npredicates].relb_id,&predicates[npredicates].colb_id);
			npredicates++;
		}
		else {
			sscanf(token,"%u.%u%c%llu",&filters[nfilters].rel_id,&filters[nfilters].col_id,
									   &filters[nfilters].cmp,&filters[nfilters].value);
			nfilters++;
		}
		token=strtok(NULL,"&");
	}
